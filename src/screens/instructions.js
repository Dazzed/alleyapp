    ...
    return (
      <View style={style.container}>
        <ScrollView keyboardShouldPersistTaps='handled'>
          {type === "textOverlay" ? <View key={"text"}><Text>{data}</Text></View> : ''}

          {(type === "audioOverlay") ? <View style={style.backgroundVideo} key={"audio"}><Video source={{ uri: data }}
            audioOnly={true}
          ...
          {(type === "videoOverlay") ? <View style={style.backgroundVideo} key={"video"}><Video source={{ uri: data }}
